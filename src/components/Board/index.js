    ...
    if (!this.state.loading) {
      ...
      if (!this.state.error) {
        if (type === 'group') {
          ...
          rowsTemp = orderBy(rowsTemp, [row => (row.destinyUserInfo.triumphScore ? row.destinyUserInfo.triumphScore : 0), row => row.destinyUserInfo.displayName.toString().toLowerCase()], ['desc', 'asc']);
