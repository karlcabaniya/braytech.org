...
export async function getGroupMembers(group) {
  ...
  let memberResponses = await Promise.all(
    groupMembersResponse.results.map(async member => {
      try {
        const [profile, historicalStats] = await Promise.all([bungie.memberProfile(member.destinyUserInfo.membershipType, member.destinyUserInfo.membershipId, '100,200,202,204,900'), bungie.getHistoricalStats(member.destinyUserInfo.membershipType, member.destinyUserInfo.membershipId, '1', '4,5,7,64', '0')]);
